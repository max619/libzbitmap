...
#if defined(LIBZBITMAP_SHARED)
    #if _WIN32
        #if defined(LIBZBITMAP_EXPORT)
            #define LIBZBITMAP_API __declspec(dllexport)
        #else
            #define LIBZBITMAP_API __declspec(dllimport)
        #endif
    #else
        #if defined(LIBZBITMAP_EXPORT)

            #define LIBZBITMAP_API __attribute__((visibility("default")))
        #else
            #define LIBZBITMAP_API
        #endif
    #endif
#else
    #define LIBZBITMAP_API
 ...
 * code in test/test.c.
 */
zbm_error_t LIBZBITMAP_API zbm_compress_chunk(void *dest, size_t dest_size, const void *src, size_t src_size, size_t index,
                                              size_t *out_len);
